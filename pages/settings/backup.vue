<template>
  <section class="page-pad fast-fade">
    ...
    <div class="grid gap-6 xl:grid-cols-4">
      ...
      <div class="card p-6"><h2 class="text-xl font-black mb-4 text-red-400">إعادة ضبط المصنع</h2><p class="text-muted mb-4">يمسح كل البيانات التشغيلية بالكامل، بما فيها سجلات الشراء والبيع والمخزن والعملاء والأقساط، مع إبقاء حساب المدير فقط.</p><FormField label="رمز التأكيد" hint="اكتب RESET-AUTODEALER بالضبط"><input v-model="confirmCode" class="input" placeholder="RESET-AUTODEALER"></FormField><button class="btn-danger btn mt-4 w-full" :disabled="busy" @click="factoryReset">مسح كل البيانات</button></div>
...
async function factoryReset(){ if(confirmCode.value!=='RESET-AUTODEALER') return notify('رمز التأكيد غير صحيح','error'); if(!confirm('هل أنت متأكد؟ سيتم حذف سجلات الشراء والبيع والمخزن والعملاء والأقساط وكل البيانات التشغيلية مع إبقاء حساب المدير فقط.')) return; busy.value=true; try{ await $fetch('/api/settings/reset',{method:'POST',body:{confirm:confirmCode.value}}); confirmCode.value=''; await refresh(); notify('تم مسح كل البيانات التشغيلية مع إبقاء حساب المدير فقط') }catch(e:any){notify(e?.data?.message||'فشل مسح البيانات','error')}finally{busy.value=false} }
</script>
