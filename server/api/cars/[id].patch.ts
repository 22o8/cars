...
export default defineEventHandler(async(event)=>{ const user=await requirePermission(event,'cars'); const id=getRouterParam(event,'id')!; const data={...schema.parse(await readBody(event)), currency:'USD' as const}; const car=await prisma.car.update({where:{id},data}); await audit(user.fullName,'تعديل سيارة','Car',id,`${car.brand} ${car.model}`); return car })
