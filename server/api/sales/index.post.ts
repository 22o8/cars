...
const schema=z.object({
  ...
  salePrice:z.number().positive(), firstPayment:z.number().min(0).default(0), currency:z.enum(['IQD','USD']).default('USD'),
...
export default defineEventHandler(async(event)=>{
 ...
 const sale=await prisma.$transaction(async(tx)=>{
  ...
  const s=await tx.sale.create({ data:{ carId:b.carId, customerId:b.customerId, soldByUserId:user.id, saleType:b.saleType, salePrice:b.salePrice, firstPayment:normalizedFirstPayment, paidAmount:paidInitial, remainingAmount:remaining, currency:'USD', notes } })
  await tx.car.update({ where:{id:b.carId}, data:{status:'SOLD'} })
  if(normalizedFirstPayment>0){ await tx.payment.create({data:{saleId:s.id, amount:normalizedFirstPayment, currency:'USD', notes:'الدفعة الأولى'}}); await tx.cashboxTransaction.create({data:{type:'INCOME', amount:normalizedFirstPayment, currency:'USD', description:'دفعة أولى من بيع سيارة', referenceId:s.id}}) }
  if(b.saleType==='TRADE_IN' && tradeValue>0){ await tx.cashboxTransaction.create({data:{type:'INCOME', amount:tradeValue, currency:'USD', description:'قيمة سيارة مراوسة محسوبة ضمن البيع', referenceId:s.id}}) }
  ...
  const invNo=`SALE-${Date.now()}`; await tx.invoice.create({data:{invoiceNumber:invNo, invoiceType:'فاتورة بيع سيارة', saleId:s.id, customerName:customer.fullName, customerPhone:customer.phone, title:`بيع ${car.brand} ${car.model} ${car.year}`, amount:b.salePrice, currency:'USD', notes:`المدفوع: ${paidInitial} | المتبقي: ${remaining}`}})
  return s
 })
